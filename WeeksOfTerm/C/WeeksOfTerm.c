...
int main(int argc, char *argv[])
{
  ...
  cal_diffdays = (int)((now - termbegin_sec)/(60*60*24)) + dayofweek(termbegin.tm_wday);		// cal_diffdays starts from first week's Monday.
  weeks = cal_diffdays/7 + ((cal_diffdays%7)?1:0);		// odd out of 7 regards as a week.
